...
## Step 3 — Download compose.yml and create `.env`
...
cat > .env <<EOF
HAILFREQ_DOMAIN=radio.your-guild.com
HAILFREQ_SERVER_HOSTNAME=server.your-guild.com
HAILFREQ_ADMIN_EMAIL=admin@your-guild.com
...
- `HAILFREQ_DOMAIN` — the identity domain users connect to (forms `@user:DOMAIN` Matrix IDs).
- `HAILFREQ_SERVER_HOSTNAME` — the hostname where Synapse and LiveKit actually run (can equal `HAILFREQ_DOMAIN` for a single-host setup).
- `HAILFREQ_ADMIN_EMAIL` — used for Let's Encrypt registration.
- `HAILFREQ_PUBLIC_IP` — your VPS's public IPv4 (the command above sets it automatically; verify with `curl ifconfig.me`).

All cryptographic secrets (Postgres password, Synapse keys, LiveKit API key/secret, TURN shared secret) are auto-generated by the `bootstrap` container on first start and stored in a managed `hailfreq-secrets` Docker volume. You do not set them in `.env`.

**Optional:** To wire CitizenID SSO, add `CITIZENID_CLIENT_ID` and `CITIZENID_CLIENT_SECRET` to `.env` before starting. See [`citizenid-setup.md`](citizenid-setup.md).
...
**Image:** Pulled from `docker.io/hailfreq/lk-auth` or `ghcr.io/hailingfrequency/livekit-auth`. To force a fresh pull: `docker compose pull livekit-auth && docker compose up -d livekit-auth`.
...
**CitizenID:** if configured, members just click "Sign in with Citizen iD" on the login screen and an account is auto-created on first login. See [`citizenid-setup.md`](citizenid-setup.md) for attribute requirements and configuration details.
